# Day 4 & 5 - March 19, 2026 & March 20, 2026

## Creating Tokens on CKB

### What i learned
- What tokens are 
- What fungible means and what fungible tokens are 
- Just as the Ethereum fungible tokens followa the ERC20 standards, on CKB, the equivalent is called xUDT (Extensible User Defined Tokens)
- Tokens are stored inside cells 
- Unlike many blockchains CKB tokens are controlled by scripts that allows you to define:
  - Who can Mint tokens
  - Who can transfer tokens
  - Supply rules 
  - Permissions 


### Today's Task 
- Create a fungible token


### What I did
- When through the documentation on creating a fungible token 
- understood the xUDTs 
- Set up the create-token exercise inside my project
- created my very own custom tokens on the CKB Devnet
- Checked the token on the Chain UI using the xUDT args
- Moved some tokens from Account #0 to Account #1

### Proof of Work 
- Adding a custom amount of Token:
![Screenshot 7](../screenshots/Screenshot%202026-03-19%20183616.png)

- Viewing the Token using the xUDT args:
![Screenshot 7](../screenshots/Screenshot%202026-03-19%20192251.png)

- Transfering some tokens to another account:
![Screenshot 7](../screenshots/Screenshot%202026-03-19%20192313.png)

Transaction hash: 0x635d9f7dc8437824a9e8cfcac792b66638c29e7d291650e348f3032f45284415
Token xUDT args: 0x7de82d61a7eb2ec82b0dc653e558ba120efcbfbb44dac87c12972d05bf25065300000000 


